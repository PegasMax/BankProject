...
from src.table_integration import get_transactions_from_excel_file, get_transactions_from_csv_file
...
def main():
    ...
    while loader_index not in ('1', '2', '3'):
        ...
        loader_index = input('''Привет! Добро пожаловать в программу работы 
...
Доступные для фильтровки статусы: EXECUTED, CANCELED, PENDING\n''').upper()
    ...
    if user_input == 'Нет':
        is_only_rubble = False
        logger.info('Выбраны все операции')
    else:
        is_only_rubble = True
        transaction_list = [transaction for transaction in transaction_list if transaction['operationAmount']['currency']['code'] == 'RUB']
    ...
    if user_input == 'Нет':
        is_description_filter = False
        logger.info('Решено не фильтровать операции')
    else:
        is_description_filter = True
    ...
    for transaction in transaction_list:
        ...
        print(f'Сумма: {round(float(transaction["operationAmount"]["amount"]))} {transaction["operationAmount"]["currency"]["name"]}\n')
