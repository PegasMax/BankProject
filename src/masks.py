...
def get_mask_card_number(card_number: int | str) -> str:
    ...
    logger.info("Начинает работу 'get_mask_card_number'")
    ...
    for char in card_number_str:
        if not ("0" <= char <= "9"):
            logger.error("В номере карты ошибка: он состоит не только из цифр.")
            raise ValueError("Номер карты может состоять только из цифр")

    # Проверка длины номера карты
    if not 12 < len(card_number_str) < 20:
        logger.error("В номере карты ошибка: он слишком короткий.")
        raise ValueError(
    ...
    logger.info("Номер карты успешно замаскирован, функция прекращает работу.")
...
def get_mask_account(account_number: int | str) -> str:
    ...
    logger.info("Начинает работу 'get_mask_account'")
    ...
    if len(account_number_str) < 4:
        logger.error("В номере счета ошибка: он слишком короткий.")
    ...
    logger.info("номер счета успешно замаскирован, функция прекращает работу")
