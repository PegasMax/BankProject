...
def get_transactions_from_excel_file(path: str) -> list[dict]:
    ...
    try:
        ...
        logger.error(f"Ошибка открытия файла {path}.")
        return []
...
def standardize_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info("Начинает работу функция приведения DataFrame к стандартному виду")
    ...
    logger.info("Функция приведения DataFrame к стандартному виду успешно завершила работу")
    return new_df


print(get_transactions_from_excel_file("../data/transactions_excel.xlsx"))
