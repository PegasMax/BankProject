...
def test_get_mask_card_number_wrong_length(card_number):
    with pytest.raises(ValueError) as exc_info:
        ...
        assert  exc_info.value == 'Номер карты может содержать от 13 до 19 цифр, проверьте корректность введенного номера карты'
...
def test_get_mask_card_number_other_symbols(card_number):
    with pytest.raises(ValueError) as exc_info:
        get_mask_card_number(card_number)
        assert exc_info.value == 'Номер карты может состоять только из цифр'
...
def test_get_mask_account_different_formats(account_number):
    with pytest.raises(ValueError) as exc_info:
        ...
        assert exc_info.value == 'Обычно номер счета содержит 20 цифр, проверьте корректность введенного номера счета'
